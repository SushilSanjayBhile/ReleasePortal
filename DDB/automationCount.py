...
@csrf_exempt
def AutomationCount1(request, Release):
    platformList = []
    release = Release
...
def AutomationCount(request, Release):
    ...
    for tc in infoserializer.data:
        if tc["CardType"]:
            ...
            if "Automated_TCs" not in dict1[tc["CardType"]]:
                dict1[tc["CardType"]]["Automated_TCs"] = 0
            if tc["TcName"] != "TC NOT AUTOMATED":
                dict1[tc["CardType"]]["Automated_TCs"] += 1

            prior = tc["Priority"]
            if prior == "P0" or prior == "P1":
                key_total = prior + "_Total"
                if key_total not in dict1[tc["CardType"]]:
                    dict1[tc["CardType"]][key_total] = 1
                else:
                    dict1[tc["CardType"]][key_total] += 1

                key_automated = prior + "_Automated"
                if key_automated not in dict1[tc["CardType"]]:
                    dict1[tc["CardType"]][key_automated] = 0
                if tc["TcName"] != "TC NOT AUTOMATED":
                    dict1[tc["CardType"]][key_automated] += 1
    dict2 = {}
    tempList = []
    print(dict1,"\n\n")
    for platform in dict1:
...
@csrf_exempt
def AutomationCountByDomain1(request, Platform):
    ...
    for tc in infoserializer.data:
        if tc["CardType"]:
            ...
            if dom == "Additional":
                if tc["CardType"]  == "DMC":
                    print(tc)
            if dom not in dict1[tc["CardType"]]:
                dict1[tc["CardType"]][dom] = {}
            if "Total_TCs" not in dict1[tc["CardType"]][dom]:
                dict1[tc["CardType"]][dom]["Total_TCs"] = 1
            else:
                dict1[tc["CardType"]][dom]["Total_TCs"] += 1
            if "Automated_TCs" not in dict1[tc["CardType"]][dom]:
                dict1[tc["CardType"]][dom]["Automated_TCs"] = 0
            if tc["TcName"] != "TC NOT AUTOMATED":
                dict1[tc["CardType"]][dom]["Automated_TCs"] += 1

            prior = tc["Priority"]
            if "P0_Total" not in dict1[tc["CardType"]][dom]:
                dict1[tc["CardType"]][dom]["P0_Total"] = 0
            if "P1_Total" not in dict1[tc["CardType"]][dom]:
                dict1[tc["CardType"]][dom]["P1_Total"] = 0
            if "P0_Automated" not in dict1[tc["CardType"]][dom]:
                dict1[tc["CardType"]][dom]["P0_Automated"] = 0
            if "P1_Automated" not in dict1[tc["CardType"]][dom]:
                dict1[tc["CardType"]][dom]["P1_Automated"] = 0

            if prior == "P0" or prior == "P1":
                key_total = prior + "_Total"
                     
                if key_total not in dict1[tc["CardType"]][dom]:
                    dict1[tc["CardType"]][dom][key_total] = 1
                else:
                    dict1[tc["CardType"]][dom][key_total] += 1

                key_automated = prior + "_Automated"
                if key_automated not in dict1[tc["CardType"]][dom]:
                    dict1[tc["CardType"]][dom][key_automated] = 0
                if tc["TcName"] != "TC NOT AUTOMATED":
                    dict1[tc["CardType"]][dom][key_automated] += 1
    dict2 = {}
    tempList = []
...
@csrf_exempt
def AutomationCountForGUI1(request, Release):
    platformList = []
    release = Release
    atd = {}
...
@csrf_exempt
def AutomationCountByDomainForGUI1(request,Release,Platform):
    infodata = TC_INFO_GUI.objects.all().using(rootRelease)
    infoserializer = TC_INFO_GUI_SERIALIZER(infodata, many = True)
