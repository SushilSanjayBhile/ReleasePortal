...
def Migrate(request, typ):
    ...
    cmd = "python3 /release-portal/app/manage.py makemigrations"
    ...
    for db in settings.DATABASES:
        if typ == "fake":
            cmd = "python3 /release-portal/app/manage.py migrate --database='"+ db +"' --fake"
            print("\n",cmd)
            os.system(cmd)
            flag = 1
        if typ == "real":
            cmd = "python3 /release-portal/app/manage.py migrate --database='"+ db +"'"
            print("\n",cmd)
            os.system(cmd)
            flag = 1
