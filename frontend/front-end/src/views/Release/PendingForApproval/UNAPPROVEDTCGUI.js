...
import {
    Col, Row, Button,Modal, ModalHeader, ModalBody, ModalFooter, Input, FormGroup, Label, Collapse, UncontrolledPopover, PopoverBody
} from 'reactstrap';
...
import { wsPUA } from '../../../constants';

class UNAPPROVEDTCGUI extends Component {
    workingStatusOptions = [{ value: 'Select Status', text: 'Select Status' },{ value: 'CREATED', text: 'CREATED' }, { value: 'DELETED', text: 'DELETED' },{ value: 'UNAPPROVED', text: 'UNAPPROVED' }];
    editedRows = {};
    isAnyChanged = false;
    ...
    //     axios.get('/api/' + this.props.selectedRelease.ReleaseNumber + '/tcinfogui/domain/' + domain)
    //         .then(all => {
    ...
    toggleAllUNAPPROVEDGUI = () => {
        this.setState({ multipleChanges: !this.state.multipleChanges },()=>{
        })
    };
    popoverToggleUNAPPROVEDGUI = () => this.setState({ popoverOpenUNAPPROVEDGUI: !this.state.popoverOpenUNAPPROVEDGUI },()=>{
    });
    popoverToggleUNAPPROVEDGUI = () => this.setState({ popoverOpenUNAPPROVEDGUI : !this.state.popoverOpenUNAPPROVEDGUI });
    reset() {
    ...
    textFields = [
        ...
        'Description', 'Steps', 'ExpectationBehavior', 'Notes','CardType','Reason'
    ];
    ...
    saveAll() {
        ...
        selectedRows.forEach(item => {
            ...
            ['WorkingStatus'].map(each => {
                if (item[each]) {
                    ...
                    if(item[each] ==  'CREATED'){
                        pushable.stateUserMapping = { 'CREATED' : `${currentUser}` }
                    }
                    ...
                    if(item[each] == 'DELETED'){
                        pushable.stateUserMapping = {'DELETED':`${currentUser}`}
                    }
                }
            })
            items.push(pushable);
        ...
        .then(res => {
            ...
            alert('Tc Updated Successfully');
        }, error => {
            this.gridOperations(true);
            alert('Failed To Update TC ',error);
        });
    }

    // Save Single Tc
    save() {
        if(this.state.approveState ==""){
            this.state.approveState = 'CREATED'
        }
        let data = {};
        ...
        data.Activity={
            ...
            "StatusChangeComments": data.WorkingStatus === 'DELETED' ? this.state.reasonForUnapproval : ''
        };

        let currentUser = this.props.tcDetails.Assignee;
        ...
        if(this.state.approveState == 'DELETED'){
            data.stateUserMapping = { 'DELETED' : `${currentUser}`}
        }
    ...
    render() {
        ...
        return (
            <div>
                                ...
                                <div class='col-lg-12'>
                                    <div style={{ display: 'flex' }}>
                                        <div onClick={() => {
                                            ...
                                            <span className='rp-app-table-title'>UnApproved Test Cases</span>
                                        ...
                                        <div class="row">
...

                                            {
                                                ...
                                                <div style={{ width: '8rem', marginLeft: '0.5rem' }}>
                                                    <span>
                                                        <Button  id="PopoverAssignUNAPPROVEDGUI" type="button">Apply Multiple</Button>
                                                        <UncontrolledPopover trigger="legacy" placement="bottom" target="PopoverAssignUNAPPROVEDGUI" id="PopoverAssignButtonUNAPPROVEDGUI" toggle={() => this.popoverToggleUNAPPROVEDGUI()} isOpen={this.state.popoverOpenUNAPPROVEDGUI}>
                                                            <PopoverBody>
                                                                {
                                                                    [
                                                                        { labels: 'WorkingStatus', values: [{ value: '', text: 'Select Working Status' }, ...(wsPUA.map(each => ({ value: each, text: each })))] },
                                                                ...
                                                                <div style={{ float: 'right', marginBottom: '0.5rem' }}>
                                                                    ...
                                                                    <span>
                                                                        {
                                                                            this.isAnyChanged &&
                                                                            <Button  title="Save" size="md" className="rp-rb-save-btn" onClick={() => { this.popoverToggleUNAPPROVEDGUI(); this.toggleAllUNAPPROVEDGUI() }} >
                                                                                Save
                                                                            </Button>
                                                                        }
                                                                    </span>
                                    ...
                                    }
                                    {
                                        ...
                                        <FormGroup row className="my-0" style={{ marginTop: '1rem' }}>
                                            ...
                                            <FormGroup row className="my-0">
                                                {
                                                    [
                                                        { field: 'UnapproveTCReason', header: 'UnapproveTCReason', type: 'text' },
                ...
                <Modal isOpen={this.state.multipleChanges} toggle={() => this.toggleAllUNAPPROVEDGUI()}>
                    {
                        <ModalHeader toggle={() => this.toggleAllUNAPPROVEDGUI()}>{
                            'Confirmation'
                        }</ModalHeader>
                    }
                    <ModalBody>
                    ...
                    <ModalFooter>
                        <Button color="primary" onClick={() => { this.toggleAllUNAPPROVEDGUI(); this.saveAll(); }}>Ok</Button>{' '}
                        {
                            <Button color="secondary" onClick={() => this.toggleAllUNAPPROVEDGUI()}>Cancel</Button>
                        }
                    </ModalFooter>
                </Modal>
...
export default connect(mapStateToProps, { saveUserPendingApproval, saveTestCase, getCurrentRelease, saveSingleTestCase, updateTCEdit })(UNAPPROVEDTCGUI);
