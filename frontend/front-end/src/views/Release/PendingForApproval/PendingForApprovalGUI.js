...
class PendingForApprovalGUI extends Component {
    workingStatusOptions = [{ value: 'Select Status', text: 'Select Status' },{ value: 'CREATED', text: 'CREATED' },{ value: 'APPROVED', text: 'APPROVED' }, { value: 'UNAPPROVED', text: 'UNAPPROVED' }];
    editedRows = {};
    isAnyChanged = false;
    ...
    constructor(props) {
        super(props);
        this.state = {
            PendingForApprovalDataList: false,
    ...
    getTcs(CardType, domain, subDomain, priority, selectedRelease) {
        ...
        // console.clear();
        console.log("url for pending for approval GUI tc",url)
        axios.get(url)
        .then(response => {
            console.log("getTcs",response.data,response.data.length)
            this.setState({
    ...
    saveAll() {
        ...
        selectedRows.forEach(item => {
            ...
            if(item.CurrentStatus.Result){
                tcResult = item.CurrentStatus.Result
                console.log("tc result undefine",tcResult)
            }
            else{
                tcResult = ''
                console.log("tc result undefined else",tcResult)
            }
            ...
            ['WorkingStatus'].map(each => {
                if (item[each]) {
                    ...
                    if(item[each] ==  'APPROVED'){
                        if(tcResult === "" || tcResult == 'undefined'){
                            ...
                            console.log("coming in else",tcResult)
    ...
    render() {
        ...
        console.log("PendingForApprovalDataList",this.state.PendingForApprovalDataList)
        return (
            <div>
                <Row>
                                ...
                                <div class='col-lg-12'>
                                    <div style={{ display: 'flex' }}>
                                        <div onClick={() => {
                                            ...
                                            <span className='rp-app-table-title'>GUI Test Cases Pending For Approval</span> &nbsp;&nbsp;&nbsp;&nbsp;
                                            {
                                        ...
                                        <div class="row">
                                            ...
                                            }
                                            {
                                                ...
                                                <div style={{ width: '8rem', marginLeft: '0.5rem' }}>
                                                    <span>
                                                        <Button  id="PopoverAssignCREATEDGUI" type="button">Apply Multiple</Button>
                                                        <UncontrolledPopover trigger="legacy" placement="bottom" target="PopoverAssignCREATEDGUI" id="PopoverAssignButtonCREATEDGUI" toggle={() => this.popoverToggleCREATED()} isOpen={this.state.popoverOpenCREATED}>
                                                            <PopoverBody>
                                                                {
                                                                    [
...
export default connect(mapStateToProps, { saveUserPendingApproval, saveTestCase, getCurrentRelease, saveSingleTestCase, updateTCEdit })(PendingForApprovalGUI);
