...
class ClosedBugReport extends Component {
    pageNumber = 0;
    startAt = 0;
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            cusReportColumnDefs: [
                ...
                cusReportColumnDefDict['ClosedBugs'],
    ...
    getTcs(startAt) {
        ...
        let promises = []
        axios.get(`/rest/AllClosedBugCount`).then(all => {
            this.maxResult = all.data.total
            for(let i = 0; i <= this.maxResult; i=i+100){
                promises.push(axios.get(`/rest/AllClosedBugs`,{
                    params: {
                        "startAt": i,
                    }
                }).then(all => {
                                ...
                                <div class='col-lg-12'>
                                    <div style={{ display: 'flex' }}>
                                        <div style={{ display: 'inlineBlock' }}>
                                            ...
                                            <span className='rp-app-table-title'>CLOSED BUG REPORT</span>
                                        ...
                                        <div class="row">
                                            ...
                                            </div>
                                            {
                                             ...
                                             <div style={{ width: '2.5rem', marginLeft: '0.5rem' }}>
                                                <Button /*disabled={this.state.isApiUnderProgress}*/ id="PopoverAssign2MRCB" type="button"><i class="fa fa-filter" aria-hidden="true"></i></Button>
                                                <UncontrolledPopover trigger="legacy" placement="bottom" target="PopoverAssign2MRCB" id="PopoverAssignButton2MRCB" toggle={() => this.popoverToggle2()} isOpen={this.state.popoverOpen2}>
                                                    <PopoverBody>
                                                        <div>
                                            ...
                                            <div style={{ width: '5rem', marginLeft: '2rem' }}>
                                                <Button disabled={this.state.isApiUnderProgress} title="Only selected Bugs will be downloaded" size="md" className="rp-rb-save-btn" onClick={() => {
                                                    ...
                                                    // }
                                                    if (this.gridApi) {
                                                        this.gridApi.exportDataAsCsv({ allColumns: true, onlySelected: false, fileName: 'Closed_Bug_List.csv', });
                                                    }
                                                    if(this.reportGridApi)
                                                    {
                                                        this.reportGridApi.exportDataAsCsv({allColumns: true, onlySelected: false, fileName: 'Closed_Bug_Summary.csv'});
                                                    }
                                                }} >
                                                    Download
                                                </Button>
...
export default (ClosedBugReport);