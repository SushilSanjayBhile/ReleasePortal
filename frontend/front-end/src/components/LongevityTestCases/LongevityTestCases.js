...
class LongevityTestCases extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            columnDefs: [
                ...
                {
                    headerName: "Duration (days)", field: "NoOfDuration", editable: true, sortable: true, filter: true, cellStyle: this.renderEditedCell, cellClass: 'cell-wrap-text',
    ...
    getTcs() {
        ...
        let url = `/api/sanity/longevity/${this.props.selectedRelease.ReleaseNumber}`;
        axios.get(url)
            .then(all => {
                // Filters should not go away if data is reloaded
                //this.setState({ domain: this.state.domain, subDomain: this.state.domain, CardType: this.state.CardType, data: null, rowSelect: false })
